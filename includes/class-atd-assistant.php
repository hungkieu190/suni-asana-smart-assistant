...
Lưu ý: Dùng emoji phù hợp, trình bày ngắn gọn nhưng phải ĐẦY ĐỦ số lượng như yêu cầu.";
	...
	public function ajax_ai_draft_comment() {
		...
		$assignee_gid = isset( $task['assignee']['gid'] ) ? $task['assignee']['gid'] : '';
		...
		if ( ! empty( $task['due_on'] ) ) {
			$due_dt = $task['due_on'];
			if ( $due_dt < $today ) {
				$diff = ( new DateTime( $today ) )->diff( new DateTime( $due_dt ) )->days;
				$due_status_context = "⚠️ CẢNH BÁO: Task này đã QUÁ HẠN {$diff} ngày (Hạn: {$due_dt}, Hôm nay: {$today}). Hãy viết bình luận với thái độ NGHIÊM TÚC, yêu cầu giải trình lý do chậm trễ VÀ đề xuất/yêu cầu cập nhật lại Hạn chót (Due date) mới ngay lập tức.";
			} elseif ( $due_dt === $today ) {
				$due_status_context = "⏰ Task ĐẾN HẠN HÔM NAY. Nhắc nhở tập trung hoàn thành.";
			} else {
				$diff = ( new DateTime( $due_dt ) )->diff( new DateTime( $today ) )->days;
				$due_status_context = "📅 Task còn {$diff} ngày nữa tới hạn. Nhắc nhẹ tiến độ.";
			}
		} else {
			$due_status_context = "⚠️ Task KHÔNG CÓ HẠN CHÓT (Due date). Hãy yêu cầu người phụ trách bổ sung ngày hoàn thành dự kiến.";
		}

		$mention_instruction = "";
		if ( $assignee_gid ) {
			$mention_instruction = "LƯU Ý QUAN TRỌNG: Hãy CHẮC CHẮN MỞ ĐẦU câu nhắc nhở bằng cụm thẻ HTML này: <a data-asana-gid=\"{$assignee_gid}\"></a> để tag trực tiếp người phụ trách. Ví dụ: '<a data-asana-gid=\"{$assignee_gid}\"></a>, tiến độ công việc này sao rồi bạn?'";
		}

		$prompt = "Hãy đóng vai là {$this->user_name}, một người quản lý chuyên nghiệp và lịch sự nhưng kiên quyết nếu cần.
...
- Hạn chót gốc: {$due}";
	...
	public function ajax_ai_post_comment() {
		...
		// Đừng dùng sanitize_textarea_field ở đây vì nó có thể làm mất thẻ <a> mention
		// Thay vào đó dùng wp_kses với luật cho phép thẻ a và data-asana-gid
		$allowed_html = array(
			'a' => array(
				'data-asana-gid' => array(),
				'href' => array(),
				'title' => array(),
			),
		);
		$text = isset( $_POST['comment_text'] ) ? wp_kses( $_POST['comment_text'], $allowed_html ) : '';
		...
		// Convert newlines to <br> for HTML rendering in Asana, then pass $is_html = true
		$html_content = nl2br( $text );
		$response     = $asana_api->create_story( $task_gid, $html_content, true );
